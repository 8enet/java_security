...
public class IPSeeker {
    ...
    private IPSeeker() {
        ...
        try {
            ...
            error("IP地址信息文件格式有错误，IP显示功能将无法使用");
            ipFile = null;
            return;
        }
     ...
     * 根据地点查ip区间
     */
    public List<IPEntry> getIPEntries(String country,String area,int limit) {
        ...
        for (int offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
            ...
            if (temp != -1) {
                ...
                if (ipLoc.country.contains(country)
                        && (area == null || ipLoc.area.contains(area)) ) {
