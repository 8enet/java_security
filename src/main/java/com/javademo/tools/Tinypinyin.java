...
public class Tinypinyin {
    ...
    public static void main(String[] args){
        ...
        int offset = c - MIN_VALUE;

        offset=3;

        int index1 = offset / 8; //找到padding数组对应的索引
        int index2 = offset % 8; //找到padding数组对应的索引值的第几位

        System.out.println("offset: "+offset+"  index1: "+index1+"  index2: "+index2);

        short realIndex = (short) (PINYIN_CODE[offset] & 0xff); //前8位

        System.out.println(realIndex);
        System.out.println("PINYIN_CODE\t"+byteToBit(PINYIN_CODE[offset]));


        System.out.println("PADDING\t\t"+byteToBit(PINYIN_CODE_PADDING[index1]));
        System.out.println("BIT_MASKS\t"+byteToBit(Integer.valueOf(BIT_MASKS[index2]).byteValue()));

        //System.out.println(byteToBit(Integer.valueOf((PINYIN_CODE_PADDING[index1] & BIT_MASKS[index2])).byteValue()));

        //根据padding判断是否需要加1位,<=255的数只需要前8位即可,大于1<<7  && 小于1<<8的
        if ((PINYIN_CODE_PADDING[index1] & BIT_MASKS[index2]) != 0) {
            System.out.println("==");

            //256~512之间的数有9位,将最高位置为1即可
            realIndex = (short) (realIndex | PADDING_MASK);
        }



        System.out.println("realIndex: "+realIndex);
        System.out.println(PINYIN_TABLE[realIndex]);
        System.out.println("-----------");

        for (int i:BIT_MASKS){
            System.out.println(byteToBit(Integer.valueOf(i).byteValue()));
        }

        System.out.println(byteToBit((byte) (1 << 7)));
    ...
    static final short PADDING_MASK = 256;
